...
if (!isset($_SESSION['id_voyageur'])) {
    // Rediriger vers la page de connexion si l'utilisateur n'est pas connecté
    header("Location: ../../index.php");
    exit;
}

// L'utilisateur est connecté, afficher son identifiant
$userId = $_SESSION['id_voyageur'];
//echo "Bienvenue, votre ID est : $userId";
?>
<!doctype html>
<html lang="en">
  <head>
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav">
        <li class="nav-item">
          ...
          <a class="nav-link active" aria-current="page" href="historique.php?choix=<?=$userId?>">Mes voyages</a>
                ...
                <img src="../../<?=$train->image;?>">
                    <div class='card-body'>
                       ...
                       <h5 class='card-subtitle'><?php echo $train->prix_billet; ?> €</h5>
                        <a href='reservation.php?choix=<?=$train->id_train?>&v_dep=<?=$train->V_DEPART?>&v_arr=<?=$train->V_ARRIVEE?>&h_dep=<?=$train->H_DEPART?>&user=<?=$userId?>'><button type="button" class="btn btn-primary">Réserver</button></a>
                </div>
            </div>
        </div>
    </section>
